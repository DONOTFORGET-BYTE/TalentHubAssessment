...
function filteredBooks() {
  ...
  tempBooks = tempBooks.sort((a, b) => {
    ...
    if (this.sortBy == 'title') {
      let comp_a = a.title.toLowerCase(),
        comp_b = b.title.toLowerCase()
      ...
      // Sort by author
      let comp_a = a.author.toLowerCase(),
        comp_b = b.author.toLowerCase()
...
<template>
  <main>
    <div>
      ...
      <v-select v-model="sortBy" :items="sortItems" label="SortBy" density="compact"></v-select>
        ...
        <div class="card" v-for="book in filteredBooks()" :key="book.id">
          ...
          <div class="content">
            <span>
              ...
              <h3 class="author-title">Author: {{ book.author }}</h3>
            </span>

            <div class="text-center">
              <v-btn color="red lighten-2" dark @click="book.dialog = true"> view more </v-btn>
              <v-dialog v-model="book.dialog" width="500">
                <v-card>
                  <v-card-title class="text-h5 grey lighten-2"> Summary </v-card-title>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="primary" text @click="book.dialog = false"> close </v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </div>
